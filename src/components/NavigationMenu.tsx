    ...
    <header className='fixed top-4 left-1/2 z-50 -translate-x-1/2 w-[min(98vw,900px)] transition-all duration-500'>
      ...
      <div className='rounded-full shadow-xl bg-background/80 backdrop-blur-lg border border-gray-400/30 px-8 py-3 flex items-center justify-between space-x-2 transition-all duration-500'>
        ...
        <nav className='hidden md:flex items-center space-x-8'>
          ...
          <div ref={workMenuRef} className='relative' onMouseEnter={openWorkMenu} onMouseLeave={closeWorkMenu}>
            <button
                ...
                <div className='px-6 space-y-6'>
                  ...
                  <div className='space-y-1 mb-4 mt-4 pb-4 relative'>
                    ...
                    >
                      <span>WORK</span>
                      <ChevronDown
                        className={`h-4 w-4 transition-transform duration-300 ${mobileWorkOpen ? 'rotate-180' : 'rotate-0'}`}
                      />
                    </button>
                    <div
                      className='absolute left-0 right-0 w-full h-px bg-gray-400/30 bottom-0'
                      style={{
                        left: '-1.5rem',
                        right: '-1.5rem',
                        width: 'calc(100% + 3rem)',
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 ${mobileWorkOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}
                    >
                      <div className='grid grid-cols-1 gap-3 pl-4'>
                        ...
                        <Link
                          ...
                          style={{
                            animationDelay: `${projects.length * 50}ms`,
                          }}
                          onClick={() => setIsOpen(false)}
                        >
